...
module.exports = {
    name: 'owner',
    description: 'ajouter ou retirer des owners de la list du bot',
    category:"owner",
    usage:`**${config.prefix}** owner <add/remove>`,
    async execute(client, message, args) {
        ...
        if (!config.owner.includes(message.author.id) || check == false) return message.channel.send("tu n'es pas un owner")
          ...
          if (args[0] && args[0] == 'add') {
            ...
            if (await db.get(`owner_${client.user.id}_${member.id}`) === true) {
              return message.channel.send(`${member.username} est déjà owner`)
            }
          ...
          if (args[0] && args[0] == 'clear') {
            let members = await db.all(`owner_${client.user.id}_`)
            ...
            .filter(m => m.id.startsWith("owner_"))
          
           
          
            const numOwners = membersWithId.length ? membersWithId.length : 0;
            message.channel.send(`${numOwners} ${numOwners > 1 ? "personnes ont été supprimées " : "personne a été supprimée"} des owners`);
          ...
          if (args[0] && args[0] == 'remove') {
            if (args[1]) {
              ...
              if (!member) return message.channel.send(`Aucun membre trouvé pour \`${args[1] || " "}\``)
              if (await db.get(`ownermd_${client.user.id}_${member.id}`) === null) return message.channel.send(`${member.username} n'est pas owner`)
